        ...
        if rbf_trainable:
            self.register_parameter(
                ...
                nn.parameter.Parameter(
                    torch.arange(1, n_rbf + 1, dtype=torch.float).unsqueeze(0)
                    / rbound_upper
                ),
            )
        else:
...
class BesselBasisLayer(torch.nn.Module):
    def __init__(
        self, num_rbf, rbound_upper, rbound_lower=0.0, rbf_trainable=False, **kwargs
    ):
        super().__init__()
        freq = (
...
class GaussianSmearing(nn.Module):
    ...
    def _initial_params(self):
        offset = torch.linspace(self.rbound_lower, self.rbound_upper, self.num_rbf)
...
class ExpNormalSmearing(nn.Module):
    def __init__(
        self, num_rbf, rbound_upper, rbound_lower, rbf_trainable=False, **kwargs
    ):
        super().__init__()
        self.rbound_upper = rbound_upper
        self.rbound_lower = rbound_lower
