...
class Clof_GCL(E_GCL):
    """
    Basic message passing module of ClofNet. This should be modified here.
    """
    ...
    def coord_model(
        ...
        trans = (
            coord_diff * coff[:, :1]
            + coord_cross * coff[:, 1:2]
            + coord_vertical * coff[:, 2:3]
        )
        trans = torch.clamp(
    ...
    def forward(self, h, edge_index, coord, edge_attr=None, node_attr=None):
        ...
        :param h: 位置矩阵变换得到的结果。
        :param edge_index: [source, target]
        :param coord: 就是位置矩阵。
        :param edge_attr: 边特征。
        :param node_attr:
        :return:
        ...
        row, col = edge_index
        residue = h
        radial, coord_diff, coord_cross, coord_vertical = self.coord2localframe(
            edge_index, coord
        )
        edge_feat = self.edge_model(h[row], h[col], radial, edge_attr)
