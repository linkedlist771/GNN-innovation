...
from dl_gnn.models.visnet.models.model import create_model, load_model
...
class LNNP(LightningModule):
    ...
    def step(self, batch, loss_fn, stage):
        ...
        if stage == "test":
            ...
            if self.hparams.derivative:
                self.inference_results["dy_pred"].append(
                    deriv.squeeze(-1).detach().cpu()
                )
                self.inference_results["dy_true"].append(
                    batch.dy.squeeze(-1).detach().cpu()
                )
    ...
    def training_epoch_end(self, training_step_outputs):
        ...
        if hasattr(dm, "test_dataset") and len(dm.test_dataset) > 0:
            ...
            should_reset = (
                self.current_epoch + delta + 1
            ) % self.hparams.test_interval == 0 or (
                (self.current_epoch + delta) % self.hparams.test_interval == 0
                and self.current_epoch != 0
            )
            if should_reset:
                self.trainer.reset_val_dataloader()
                self.trainer.fit_loop.epoch_loop.val_loop.epoch_loop._reset_dl_batch_idx(
    ...
    def test_epoch_end(self, outputs) -> None:
        for key in self.inference_results.keys():
            if len(self.inference_results[key]) > 0:
                self.inference_results[key] = torch.cat(
                    self.inference_results[key], dim=0
                )
