...
class MD22(InMemoryDataset):
    def __init__(self, root, dataset_arg=None, transform=None, pre_transform=None):
        ...
        super(MD22, self).__init__(
            osp.join(root, dataset_arg), transform, pre_transform
        )
    ...
    def download(self):
        ...
        download_url(
            self.base_url + self.molecule_names[self.dataset_arg], self.raw_dir
        )
    ...
    def process(self):
        for path, processed_path in zip(self.raw_paths, self.processed_paths):
            ...
            samples = []
            for pos, y, dy in tqdm(
                zip(positions, energies, forces), total=energies.size(0)
            ):
