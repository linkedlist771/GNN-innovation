...
class ViSNetBlock(nn.Module):
    ...
    def __init__(
        ...
        self.vis_mp_layers.append(
            vis_mp_class(last_layer=True, **vis_mp_kwargs).jittable()
        )
...
class ViS_MP(MessagePassing):
    def __init__(
        ...
        super(ViS_MP, self).__init__(aggr="mean", node_dim=0)
    ...
    def message(self, q_i, k_j, v_j, vec_j, dk, dv, r_ij, d_ij):
        # 这里是选取了所有i条边的。特征， 用于生成message然后进行聚合得到·的。
    ...
    def aggregate(
        ...
        agger_method = "mean"
        x, vec = features
        x = scatter(x, index, dim=self.node_dim, dim_size=dim_size, reduce=agger_method)
        vec = scatter(vec, index, dim=self.node_dim, dim_size=dim_size, reduce=agger_method)  # TODO: 修改聚合里面的求和为mean防止数值梯度爆炸
...
class ViS_MP_Vertex_Edge(ViS_MP):
    ...
    def forward(self, x, vec, edge_index, r_ij, f_ij, d_ij):
        ...
        dk = dk / (dk.norm(dim=-1, keepdim=True) + 1e-6)
        dv = dv / (dv.norm(dim=-1, keepdim=True) + 1e-6)
        vec_project = self.vec_proj(vec)
        vec_project = vec_project / (vec_project.norm(dim=-1, keepdim=True) + 1e-6)

        vec1, vec2, vec3 = torch.split(vec_project, self.hidden_channels, dim=-1)
        vec_dot = (vec1 * vec2).sum(dim=1)
        ...
        o_proj = o_proj / (o_proj.norm(dim=-1, keepdim=True) + 1e-6)

        o1, o2, o3 = torch.split(o_proj, self.hidden_channels, dim=1)
        dx = vec_dot * o2 + o3
        dvec = vec3 * o1.unsqueeze(1) + vec_out
        if not self.last_layer:
            # edge_updater_type: (vec: Tensor, d_ij: Tensor, f_ij: Tensor)
