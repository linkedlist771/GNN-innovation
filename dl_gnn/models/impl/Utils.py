...
def train_grad(data, optimizer, mod, loss_fn, ratio_y, ratio_dy):
    ...
    if np.isnan(loss_dy.item()):
        logger.warning("Nan in force in train_grad, using only energy loss")
...
def train_grad_combined_model(data, optimizer, mod, loss_fn, ratio_y, ratio_dy):
    mod.train()
    optimizer.zero_grad()
    # z是每个原子的系数(就是对应)，形状是 batch_size x atomic_number
    # pos是每个原子的坐标，形状是 batch_size x atomic_number x 3
...
def test_grad_combined_model(data, mod, score_fn, show_progress_bar=False):
    mod.eval()
    loss_y_list = []
    loss_dy_list = []

    # 使用tqdm包装数据迭代器以显示进度条
    iterable = tqdm(data, ncols=80, mininterval=1) if show_progress_bar else data

    for test_batch in iterable:
    ...
    # whether nana in force
    if np.isnan(np.average(loss_dy_list)):
        logger.warning("Nan in force in test_grad, using only energy loss")
        return np.average(loss_y_list), np.nan
    else:
        return np.average(loss_y_list), np.average(loss_dy_list)


def test_grad(data, mod, score_fn, show_progress_bar=False):
    mod.eval()
    loss_y_list = []
    loss_dy_list = []
