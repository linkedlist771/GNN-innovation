...
class QM9dipole_moment(nn.Module):
    ...
    def forward(self, z, s, pos):
        ...
        q = self.mlp(s)
        q[mask] = 0
        q = q - torch.sum(q, dim=1, keepdim=True) / torch.sum(
            1 - mask.to(torch.float), dim=1, keepdim=True
        ).unsqueeze_(-1)
        q[mask] = 0
...
class QM9electronic_spatial_extent(nn.Module):
    ...
    def forward(self, z, s, pos):
        ...
        c = torch.sum(mass * pos, dim=1, keepdim=True) / torch.sum(
            mass, dim=1, keepdim=True
        )
        ret = torch.sum(
            q.squeeze(-1) * torch.sum(torch.square(pos - c), dim=-1),
            dim=1,
            keepdim=True,
        )
        return ret
...
output_dict = {
    "scalar": QM9scalar,
    "dipole_moment": QM9dipole_moment,
    "electronic_spatial_extent": QM9electronic_spatial_extent,
}
